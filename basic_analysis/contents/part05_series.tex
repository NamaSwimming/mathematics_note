...
\begin{abstract}
この章では，数列の項を限りなく足し合わせた「級数」を導入し，その収束・発散を部分和の数列の極限として定義する．まず級数の収束に関するコーシーの収束条件を確認し，収束級数の一般項が $0$ に収束することを導く．次に，絶対収束の概念を導入し，絶対収束する級数は収束することを示す．そのうえで，級数の線型性や項のまとめ方に関する性質を確認する．最後に，各項が非負である正項級数を扱い，その収束判定に有効な比較定理を証明する．
\end{abstract}
...
\begin{theorem}{級数に関するコーシーの収束条件}{級数に関するコーシーの収束条件}
	...
	\[
		m,n \geqq n_0 \implies \abs{a_{m+1}+a_{m+2}+\cdots+a_{n}}<\varepsilon
	\]
	となることは同値である．
\end{theorem}
...
\begin{leftbar}
	\begin{proof}
		...
		\[
			m,n \geqq n_0 \implies \abs{a_{m+1}} +\abs{a_{m+2}} + \cdots + \abs{a_n}<\varepsilon
		\]
		である．ここで，絶対値の性質により，
		\[
...
\begin{theorem}{比較定理(1)}{比較定理(1)}
	\begin{align*}
		...
		& \implies  \left( \sum_{n=0}^{\infty} a_n~ {:} \text{収束} \right)
	\end{align*}
\end{theorem}
...
\begin{theorem}{比較定理(2)}{比較定理(2)}
	\begin{align*}
		...
		& \implies  \left( \sum_{n=0}^{\infty} a_n~ {:} \text{発散} \right)
	\end{align*}
\end{theorem}
...
\begin{theorem}{コーシー積}{コーシー積}
	\begin{align*}
		...
		\implies & \left (\sum_{n=0}^{\infty} \sum_{k=0}^{n} a_k b_{n-k} ~{:} \text{絶対収束} \right ) \land \sum_{n=0}^{\infty} \sum_{k=0}^{n} a_k b_{n-k} = \left( \sum_{n=0}^{\infty} a_n \right) \left( \sum_{n=0}^{\infty} b_n \right)
	\end{align*}
\end{theorem}
