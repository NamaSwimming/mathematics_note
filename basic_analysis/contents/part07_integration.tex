...
\[
   \forall \alpha \in I , ~ \forall \varepsilon >0 , ~ \exists \delta >0,~ \forall x \in I : (\abs{x-\alpha}<\delta \to \abs{f(x)-f(\alpha)}<\varepsilon)
\]
...
\begin{align*}
    & \text{$f$が$I$上で一様連続} \\
  \iff &  \forall \varepsilon >0 , ~ \exists \delta >0 ,~\forall \alpha \in I ,~ \forall x \in I : (\abs{x-\alpha}<\delta \to \abs{f(x)-f(\alpha)}<\varepsilon).
\end{align*}
\end{definition}

\begin{remark}[連続と一様連続の違い]
  一様連続の定義では，\emph{$\alpha$の位置にかかわらず$\abs{x-\alpha}<\delta$を満たす$\delta$をとることができる．}
  ふたつの定義を見比べると，連続では$\forall \alpha$が$\exists \delta$より前にあるのに対し，一様連続では$\exists \delta$が$\forall \alpha$より前に出ている．
