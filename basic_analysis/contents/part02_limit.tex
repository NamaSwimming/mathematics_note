...
\begin{abstract}
この章では，実数列の極限を厳密に定義し，その基本性質を確認する．高校で「数列の極限」を定義する際に用いられた「限りなく近づく」という表現は，厳密な議論を行ううえでは曖昧さを残すものである．そこでまず，この曖昧な表現を論理的に厳格な形へと定義し直した $\varepsilon$--$N$ 論法を導入し，実数列の収束と発散を定義する．次に，数列を自然数から実数への「写像」として捉え直し，極限の一意性を証明する．そのうえで，極限の四則演算や，極限操作が順序関係を保存することを示し，未知の数列の収束を判定する強力な手法である「はさみうちの原理」を具体例とともに確認する．このような厳密な議論は，\partref{実数の連続性}において「実数の連続性」を数学的に記述するために不可欠である．
\end{abstract}

\phantomsection
\section{実数列の極限}\label{sec:実数列の極限} \index{きょくげん@極限}
...
\begin{remark}[日本語の言い回しと論理式の対応]
  \deref{実数列の収束}のふたつの表現は，つぎのように1対1に対応している．
  \begin{center}
    \begin{tabular}{l|l}
      \hline
      日本語による表現                                  & 論理記号による表現                              \\
  ...
  順序を入れ替えて$\lex{n_0 \in \mathbb{N}} \lsep \lall{\varepsilon > 0}$としてしまうと，「$\varepsilon$によらないひとつの$n_0$がある」という，まったく別の（そしてはるかに強い）主張になってしまう．
\end{remark}
...
\begin{definition}{実数列の $+\infty$ への発散}{実数列の正の無限大への発散}
  実数列 $(a_n)_{n \in \mathbb{N}}$ が $+\infty$ に発散するとは，任意の $M \in \mathbb{R}$ に対して，ある $n_0 \in \mathbb{N}$ が存在し，すべての $n \in \mathbb{N}$ に対して $n \geqq n_0$ ならば
  \[
    a_n > M
  \]
...
\begin{remark}[「収束しない」ことの論理式]
  \deref{実数列の発散}にいう「収束しない」を，論理式の形で書いてみよう．
...
ここで，実数列を集合論的な観点から再定義しておく．
実数列は，自然数から実数への写像として捉えることができる．

\begin{definition}{実数列}{実数列}
写像 $a \colon \mathbb{N} \to \mathbb{R}$ を実数列とよぶ．
自然数 $n \in \mathbb{N}$ による像 $a(n)$ を $a_n$ と書き，数列そのものを $(a_n)_{n \in \mathbb{N}}$ と表記する．
\end{definition}

\begin{prop}{実数列の極限の一意性}{実数列の極限の一意性}
  数列 $(a_n)_{n \in \mathbb{N}}$ が収束するとき，その収束先（極限値）はただひとつに限る．
\end{prop}
...
\begin{proof}
  ...
  \]
  ここで $n \geqq \max\{n_0, n_1\}$ を満たす $n$ を任意にとると，三角不等式より
  \[
    \abs{b-a} = \abs{(b - a_n) + (a_n - a)} \leqq \abs{a_n - b} + \abs{a_n - a} < \varepsilon + \varepsilon = 2\varepsilon = \abs{b-a}
  \]
  となり，$\abs{b-a} < \abs{b-a}$ という矛盾が生じる．したがって，収束先はただひとつである．これが証明すべきことであった．
\end{proof}
\end{leftbar}
...
たとえば，
\[
a_n \coloneqq n+\sin^2 n,
\qquad
b_n \coloneqq -n+\cos^2 n+\frac{\sin n}{n}
\]
とすると，数列$(a_n)$と$(b_n)$はともに発散する．
一方，
\[
a_n+b_n
=\sin^2 n+\cos^2 n+\frac{\sin n}{n}
=1+\frac{\sin n}{n}
\to 1
\]
である．

もっとも，このことはより簡単な例からも容易にわかる．
実際，
\[
a_n\coloneqq n,
\qquad
...
\]
とすれば，$(a_n)$と$(b_n)$はともに発散するが，
\[
a_n+b_n=1+\frac1n\to 1
\]
である．

このように，数列$(a_n+b_n)$が収束するからといって，$(a_n)$と$(b_n)$がそれぞれ収束するとは限らない．
...
\begin{proof}
  ...
  \[
    \abs{a_n - a} < \varepsilon \limp a - \varepsilon < a_n < a + \varepsilon
  \]
  \[
    \abs{b_n - b} < \varepsilon \limp b - \varepsilon < b_n < b + \varepsilon
  \]
  ...
  \]
  となり，すべての $n$ で $a_n \leqq b_n$ とした仮定に矛盾する．ゆえに $a \leqq b$ である．これが証明すべきことであった．
\end{proof}
\end{leftbar}

\begin{remark}[厳密な不等号は極限で保存されない]
  注意すべき点は，元の数列の間に厳密な不等号 $a_n < b_n$ が成り立っていても，極限において不等号が維持されるとは限らないことである．
  たとえば，$a_n \coloneqq 1 - \frac{1}{n}$ および $b_n \coloneqq 1 + \frac{1}{n}$ とすると，任意の $n \in \mathbb{N}$ において $a_n < b_n$ が成立するが，極限値はともに $1$ となり，$a = b$ が成立してしまう．このように，極限操作によって不等号の「ゆとり」が消滅し，等号が成立する可能性がある．
\end{remark}

\begin{theorem}{はさみうちの原理}{はさみうちの原理} \index{はさみうちのげんり@はさみうちの原理}
...
\begin{proof}
  ...
  \end{align}
  ここで $n_0 \coloneqq \max \{ n_1, n_2 \}$ と定めると，$n \geqq n_0$ においては上の 2 つの不等式が同時に成立する．
  さらに，仮定 $a_n \leqq c_n \leqq b_n$ を合わせると， $n \geqq n_0$ において
  \[
    a - \varepsilon < a_n \leqq c_n \leqq b_n < a + \varepsilon
  \]
  が成り立つ．したがって，$n \geqq n_0$ ならば常に
  \[
    a - \varepsilon < c_n < a + \varepsilon \lequiv \abs{c_n - a} < \varepsilon
  \]
  が成立する．これは，数列 $(c_n)_{n \in \mathbb{N}}$ が $a$ に収束することの定義そのものである．これが証明すべきことであった．
\end{proof}
\end{leftbar}
