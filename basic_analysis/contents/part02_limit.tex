...
ここで，実数列を集合論的な観点から再定義しておく．数列とは，各自然数に対して一つの実数を割り当てる規則にほかならない．

\begin{definition}{実数列}{実数列}
  写像 $a \colon \mathbb{N} \to \mathbb{R}$ を実数列とよぶ．自然数 $n \in \mathbb{N}$ による像 $a(n)$ を $a_n$ と書き，数列そのものを $(a_n)_{n \in \mathbb{N}}$ と表記する．
\end{definition}
