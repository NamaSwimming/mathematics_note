...
{%
  ...
  \etoclink{\bfseries\color{mypurple}\etocnumber~\etocname}%
...
\subsubsection{論理記号}
    ...
    数学において，「すべての$n$に対して$P(n)$が成り立つ」という全称命題を否定するには，\textbf{反例}を一つ示すだけで十分である．これは，全称命題が「すべての場合において成り立つ」ことを主張しているため，一つでも例外があれば命題全体が偽となるからである．

    \begin{example}{反例による反証}{反例による反証}
        次の命題を考える：
        \[
        \forall n \in \mathbb{N}\, : \, \frac{n^3}{24} + 50 \geq n^2.
        \]
        この命題が成り立たないことを示すためには，ある$n$について不等式が成立しないことを示せばよい．実際に，$n = 15$のとき，
        \[
        \frac{15^3}{24} + 50 = \frac{3375}{24} + 50 \approx 140.625 + 50 = 190.625,
        \]
        一方，
        \[
        15^2 = 225.
        ...
        したがって，
        \[
        \frac{15^3}{24} + 50 \approx 190.625 < 225 = 15^2.
        \]
        このように，$n = 15$において不等式が成り立たないため，元の命題は偽であることが分かる．

        また，数学において反例を示す際には，その値が命題を否定することを確認すれば十分であり，$n=15$をどのように見つけたかを詳細に説明する必要はない．
    \end{example}
    ...
    \begin{example}{帰納法}{帰納法}
        $n$を自然数とする．$n$に対して，$1 + 2 + \cdots + n = \frac{n(n+1)}{2}$が成り立つことを帰納法を用いて示せ．
        
        \textbf{解答}：$n=1$のとき，$1 = \frac{1(1+1)}{2}$であるから，$n=1$のとき成り立つ．
        
        仮定として，$n=k$のとき$1 + 2 + \cdots + k = \frac{k(k+1)}{2}$が成り立つと仮定する．
        ...
        &= \frac{(k+1)(k+2)}{2}
        \end{align*}
        であるから，$n=k+1$のときも成り立つ．よって，任意の自然数$n$に対して$1 + 2 + \cdots + n = \frac{n(n+1)}{2}$が成り立つ．
      \end{example}

      \begin{example}{背理法}{背理法}
        $x$を有理数とする．$x^2 = 2$ならば，$x$は有理数であることを示せ．
        
        \textbf{解答}：$x$が有理数でないと仮定する．
        
        このとき，$x = \frac{a}{b}$と表せる．ただし，$a$と$b$は互いに素である．
        ...
        したがって，$a^2$は偶数である．よって，$a$も偶数である．
        
        しかし，$a$が偶数であるとすると，$a = 2c$と表せる．
        
        このとき，$a^2 = 4c^2$であるから，$a^2$は$4$の倍数である．
        
        しかし，$a^2 = 2b^2$であるから，$a^2$は$2$の倍数である．
        
        これは矛盾である．よって，$x$は有理数である．
      \end{example}

      \begin{example}{直接証明}{直接証明}
        $x$を実数とする．$x^2 = 0$ならば，$x = 0$であることを示せ．
        
        \textbf{解答}：$x^2 = 0$とする．
        
        このとき，$x^2 = 0$より，$x = 0$または$x = -0$である．
        
        しかし，$-0 = 0$であるから，$x = 0$である．
      \end{example}
