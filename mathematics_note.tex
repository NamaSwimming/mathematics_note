...
\newtcolorbox[auto counter, number format=\Roman]{mycolumn}{
  enhanced, breakable,
  empty,            % 既定の枠は消す（線は自前で描く）
  %---- タイトル ----
  title={\bfseries\sffamily Column \thetcbcounter},
  coltitle=colexam,
  ...
  top=4pt,
  pad at break*=0mm,
  %---- タイトルを避けて縦線を描く（interior.* を使うのがコツ）----
  frame code={
    \draw[colexam,line width=2pt]
      ([xshift=-0.5pt]interior.north east) -- ([xshift=-0.5pt]interior.south east);
    \draw[colexam,line width=2pt]
      ([xshift=-1pt]interior.north west) -- ([xshift=-1pt]interior.south west);
  },
}
...
\end{quotation}
\begin{wrapfigure}[8]{r}{0.425\textwidth}
  %\centering
