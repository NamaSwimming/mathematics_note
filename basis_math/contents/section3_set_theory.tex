...
\begin{itemize}
  \item 集合では書き並べる順序が重要でないため，たとえば $\{1, 2, 3\} = \{3, 2, 1\}$である．
  \item 同じ要素が重複して含まれていても，1つの要素として扱われるため，たとえば$\{1, 1, 2, 2, 2, 3\} = \{1, 2, 3\}$である．
  \item 集合の要素には，種類が異なるものを同時に含めることができる．たとえば，$\{4, \{3\}\}$では，$4$は数であり，$\{3\}$は集合であるが，集合としての資格がある．
\end{itemize}
...
数学を勉強するうえで，よく使う集合には固有の記号を与える場合が多い．以下ではよく使う集合の例を挙げよう．

\begin{shadebox}
  よく使う集合
  \begin{itemize}
    \item $\mathbb{R}$は実数全体の集合を表している．Real numberの頭文字をとった．
    \item $\mathbb{N}=\{1,2,3,\cdots \}$は自然数全体の集合を表している\footnote{流儀によっては自然数に$0$を含めることもあるが，本書では自然数に$0$を含めないと約束する．}．Natural number の頭文字をとった．
...
\begin{shadebox}
  ...
  \begin{itemize}
    \item $\varnothing$ \footnote{空集合は$\emptyset$と表記することもある．ギリシャ文字の$\phi$で代用されることもあるが，本来の空集合の記号はノルウェー語由来である．}は要素をひとつも持たない集合，すなわち\emph{空集合}\index{くうしゅうごう@空集合}を表している．
...
\begin{description}
  ...
  \item[内包的記法\index{ないほうてききほう@内包的記法}（条件形式）] \mbox{} \\
        $\{ x \in \mathbb{N} \mid x \leqq 5 \}$のように条件を用いて表す．
\end{description}
