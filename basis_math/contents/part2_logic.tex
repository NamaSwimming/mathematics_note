...
\begin{abstract}
この章では，数学の議論を支える「論理」を整理する．まず命題論理の記号と真理値表を確認し，交換律・分配律・ド・モルガンの法則などの基本法則を述べる．含意の真偽をめぐる注意と，逆・裏・対偶の関係もここで確認する．次に，変数を含む文を扱う述語論理を導入し，全称命題・存在命題，自由変数と束縛変数，および量化子の入れ子について注意しよう．
\end{abstract}
...
以下，ふたつの論理式$P$，$Q$について，それらに含まれる命題（命題変数）の真偽を\textbf{どのように定めても}$P$と$Q$の真理値がつねに一致するとき，$P$と$Q$は\emph{論理的に同値}\index{ろんりてきどうち@論理的同値}であるといい，$P \equiv Q$と表す\footnote{$P \leftrightarrow Q$がそれ自体ひとつの論理式であるのに対し，$P \equiv Q$は「$P \leftrightarrow Q$がつねに真である」という，論理式についての主張である点で，両者は区別される．}．

たとえば，のちに示す$P \to Q \equiv \lnot P \lor Q$は，「$P$，$Q$の真偽の組合せ$4$通りの\textbf{すべて}において，$P \to Q$と$\lnot P \lor Q$の真理値が一致する」ことを主張している．ある特定の場合にたまたま真偽が一致するだけでは，論理的に同値であるとはいわないことに注意しよう．
...
\begin{remark}[量化子つき記法の意味]
  ...
  という記法は，厳密には
  \[
    \forall x \, \bigl( x \in \mathbb{R} \to P(x) \bigr)
  \]
  ...
  は
  \[
    \exists x \, \bigl( x \in \mathbb{R} \land P(x) \bigr)
  \]
  の略記であり，こちらは含意ではなく「かつ」であることに注意したい．全称は$\to$と，存在は$\land$と組になる，というこの対応は，のちに述べる量化子の否定の規則とも整合的である\footnote{実際，$\lnot \forall x \, ( x \in \mathbb{R} \to P(x) ) \equiv \exists x \, \lnot ( x \in \mathbb{R} \to P(x) ) \equiv \exists x \, ( x \in \mathbb{R} \land \lnot P(x) )$となり，「$P(x)$を満たさない実数$x$が存在する」という正しい否定が得られる（含意の否定$\lnot(P \to Q) \equiv P \land \lnot Q$を用いた）．}．
\end{remark}
...
\]
における$x$は，量化子$\forall$によって\emph{束縛}されているといい，このような変数を\emph{束縛変数}\index{そくばくへんすう@束縛変数}という．束縛変数を含む文は，変数に値を代入する余地がすでにないので，全体として真偽の定まった命題となりうる．

束縛変数は，文字を付け替えても命題の意味が変わらない．たとえば，
\[
  \forall x \in \mathbb{R} \, : \, x^2 \geqq 0
  \qquad \text{と} \qquad
  \forall y \in \mathbb{R} \, : \, y^2 \geqq 0
\]
はまったく同じ命題である\footnote{同様の現象は，定積分$\int_0^1 x^2 \, dx = \int_0^1 t^2 \, dt$や総和$\sum_{k=1}^{n} k = \sum_{j=1}^{n} j$の変数にも見られる．これらの変数も「束縛」されており，\emph{ダミー変数}とよばれることがある．}．
