...
\begin{figure}[h]
  ...
  \begin{tikzpicture}[
    ...
    \useasboundingbox (-5.0,-2.6) rectangle (6.0,2.85);
...
\begin{example}{自動販売機}{自動販売機}
  自動販売機には複数のボタンがあり，それぞれが特定の飲み物に対応している\footnote{ボタンをひとつ押しただけで何種類も飲み物が出てくる自動販売機はまずないであろう．}．
...
\begin{figure}[h]
  ...
  \begin{tikzpicture}[
    ...
    % A' と f(A')
    \draw[subA] (-2,0) ellipse (0.75cm and 1.25cm);
    \node[black!70, anchor=east] at (-3.55,0.55) {$A'$};

    \draw[subB] ( 3,0) ellipse (0.70cm and 0.95cm);
    \node[black!70, anchor=west] at ( 4.55,0.55) {$f(A')$};
...
\begin{figure}[h]
  ...
  \begin{tikzpicture}[
    ...
    % B' と f^{-1}(B')
    \draw[subB] ( 3,0) ellipse (0.65cm and 0.95cm);
    \node[black!70, anchor=west] at ( 4.55,0.55) {$B'$};

    \draw[subA] (-2,0) ellipse (0.80cm and 1.55cm);
    \node[black!70, anchor=east] at (-3.55,0.55) {$f^{-1}(B')$};

    % 要素（座標：楕円内に収まるよう圧縮）
    \coordinate (A1) at (-2, 1.2);
    \coordinate (A2) at (-2, 0.4);
    \coordinate (A3) at (-2,-0.4);
    \coordinate (A4) at (-2,-1.2);
    ...
    \draw[map] (A1) to[out=10,  in=170] (Ba);
    \draw[map] (A2) -- (Ba);
    \draw[map] (A3) to[out=-10, in=-170] (Bb);
    \draw[map] (A4) to[out=0,   in=180] (Ba);

    % 写像名（任意）
    \node[black!70] at (0.5,1.65) {$f$};
  \end{tikzpicture}
  \caption{逆像}
\end{figure}
...
\begin{example}{電話番号と電話機}{電話番号と電話機}
  ...
  たとえば，$A = \{\text{090-1234-5678}, \text{080-9876-5432}, \text{070-1111-2222}\}$，
...
\begin{example}{分類コードと本}{分類コードと本}
  ...
  たとえば，$A = \{\text{『数学入門』}, \text{『微分積分』}, \text{『物理学基礎』}, \text{『統計学』}\}$，
  ...
  このとき，$f$は全射である．なぜなら，$B$の要素である$410$，$420$，$350$のいずれもが，$A$のある要素の像になっているからである．しかし，$f$は単射ではない．なぜなら，$\text{『数学入門』}$と$\text{『微分積分』}$がともに410に対応しており，異なる本が同じ分類コードに対応しているからである．
