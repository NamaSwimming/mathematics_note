...
\begin{abstract}
  数学の学習を始める前に，定義・命題・定理・補題・系などの基本的な用語について理解を深めたい．
  「『命題』という言葉は知っているけど，『定理』とはどう違うの？」という疑問を持ったり，「『定義』と『定理』の違いがあやふやなままで，よくわからない」と感じる学生は多い．
  高等学校までの知識で，これらの用語を再確認することを試みよう．
\end{abstract}
...
\textbf{(i) $f$ が定数の場合}：任意の $x \in [a,b]$ で $f'(x)=0$ なので任意の $c \in(a,b)$ で$f(c)=0$ となる．

\textbf{(ii) $f$ が定数でない場合}：端点では $f(a)=f(b)$ だから
$M>f(a)$ または $m<f(a)$ が成り立ち，したがって$f$の最大値（または最小値）は
内部点 $c \in(a,b)$ にて達成される．
...
\begin{proof}
  ...
  \]
  よってこのとき$f(y)=f(x)$である．．いま$x$，$y$ は任意なので $f$ は$I$上で一定である．
\end{proof}
\end{tleftbar}
...
\begin{proof}
  ...
  \]
  となる．右辺は $0$以上，かつ $y-x>0$ だから $f(y)-f(x)\ge 0$である．このことからただちに主張が従う．
  (2) も同様である．
\end{proof}
\end{tleftbar}
