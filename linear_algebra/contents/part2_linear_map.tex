...
高校で学ぶベクトルの内積からも，線型写像の例が得られる．
一方のベクトルを固定して内積をとる操作は，もう一方のベクトルについて線型になる．
...
そのためには，条件が破れるようなベクトルやスカラーをひとつ見つければ十分であり，すべての場合を調べる必要はない．
...
\begin{mycolumn}
  ...
  このような「線型写像に平行移動を加えた写像」は，線型写像を少し拡張したものと考えることができる．
  ...
  \[
    ...
    \longmapsto
    \begin{pmatrix} ax + b \\ 1 \end{pmatrix}
  \]
...
\begin{mycolumn}
  ...
  これにより，ここで数ベクトルについて学んだ線型写像の考え方を，より広い対象に対して統一的に扱えるようになる．
\end{mycolumn}
...
これらの操作によって，すでに知られている線型写像から，別の線型写像を組み立てることができる．
...
そして，このようにして得られた写像も線型写像になることを確かめよう．
...
次に，線型写像を合成する操作を考える．結論からいえば，このときの合成写像も線型写像になる．
これまでに見た和やスカラー倍に加えて，合成についても線型性が保たれるとなると
「なぜこれほど多くの操作によって写像の線型性が保たれるのか」と不思議に感じるかもしれない．
しかし，線型写像と行列との対応を念頭に置けば，
これは行列どうしの積に対応する自然な性質であるといえる．
...
この対応を具体的に記述するためには，$K^n$の各成分を一つずつ取り出して考えると都合がよい．
そのための基本的な役割を果たすのが，次に定める標準基底ベクトルである．
...
\footnote{「基底」という概念そのものは，\partref{線型空間}で正式に定義される（\deref{基底}）．
そこで見るように，$(\bm{e}_1, \bm{e}_2, \dots, \bm{e}_n)$は$K^n$の基底のもっとも代表的な例であり，
「標準基底ベクトル」という名称もそれに由来する．現段階では，「第$j$成分だけが$1$で他の成分がすべて$0$のベクトル」を表す名前だと思っておけばよい．}．
...
まとめると，行列$A$の定める写像$T_A$は線型写像であり，逆に，$K^n$から$K^m$への線型写像は，ただひとつの行列によって表される．
...
\begin{remark}[線型写像は基底ベクトルの行き先で決まる]
  ...
  実際，任意のベクトルは標準基底ベクトルの線型結合（\partref{線型独立性と階数}で定義する）として表せるので，線型性を用いれば，$T(\bm{e}_1), T(\bm{e}_2), \dots, T(\bm{e}_n)$から$T(\bm{x})$の値をすべて復元できる．
