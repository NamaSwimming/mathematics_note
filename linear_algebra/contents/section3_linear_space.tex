...
そこでまず，抽象的にベクトルを再定義するために必要な概念として，
\textbf{線型空間}\index{せんけいくうかん@線型空間}
を定義する．

なお，第1章と第2章では，スカラーの範囲$K$として$\mathbb{R}$または$\mathbb{C}$を考えるという約束をしていた．
本章では議論を一般化し，$K$は任意の\textbf{体}\index{たい@体}（field）であるとする\footnote{体とは，大まかにいえば，加法・減法・乗法，および$0$でない元による除法が自由に行える代数系のことである（$0$による除法は許されない）．$\mathbb{R}$や$\mathbb{C}$はその代表例であり，本書では，体の典型例として$\mathbb{R}$と$\mathbb{C}$を思い浮かべておけば十分である．}．
ただし，具体例の多くでは，引き続き$K = \mathbb{R}$または$K = \mathbb{C}$の場合を扱う．

線型空間の定義には，体$K$がもともともっている$2$つの演算と，
$V$に新たに定める$2$つの演算，あわせて$4$つの演算が現れる．
これらを混同しないよう，次の定義では，
$K$の演算を桃色の記号で，$V$がかかわる演算を黒の記号で表す．

\begin{definition}{線型空間}{線型空間}
  $K$を体とする．
  体$K$には，\textbf{加法}および\textbf{乗法}とよばれる演算
  \[
    \Kadd\colon K\times K\to K,
    \qquad
    \Kmul\colon K\times K\to K
  \]
  が備わっており，加法の零元を$0_K$，乗法の単位元を$1_K$で表す．

  さらに，集合$V$に，\textbf{加法}とよばれる演算
  \[
    \Vadd\colon V\times V\to V,
    \qquad
    (\bm{v},\bm{w})\mapsto\bm{v}\Vadd\bm{w}
  \]
  および\textbf{スカラー倍}とよばれる演算
  \[
    \scalmul\colon K\times V\to V,
    \qquad
    (\alpha,\bm{v})\mapsto\alpha\scalmul\bm{v}
  \]
  が定められているとする．
  これらの演算が次の条件を満たすとき，
  $V$を（これらの演算を込めて）体$K$上の\textbf{線型空間}という．

  \begin{enumerate}[label=(\roman*),leftmargin=3.2em]
    \item \textbf{加法に関して可換群}

    集合$V$は，加法$\Vadd$に関して可換群をなす．
...
しかし，どの演算であるかは，記号の左右におかれた対象が$K$の元か$V$の元かによって
文脈から判断できる．
そこで以下では，$2$つの加法をいずれも通常の演算記号$+$で表し，
$K$の乗法とスカラー倍の記号は省略する．
すなわち
\[
  \alpha\Kadd\beta,\qquad
  \bm{v}\Vadd\bm{w},\qquad
  \alpha\Kmul\beta,\qquad
  \alpha\scalmul\bm{v}
\]
を，それぞれ
\[
...
\]
と表される．
ここで，同じ記号が異なる演算を表していることに注意してほしい．
たとえば$(\alpha+\beta)\bm{v}=\alpha\bm{v}+\beta\bm{v}$では，
左辺の$+$は$K$の加法，右辺の$+$は$V$の加法である．
また$(\alpha\beta)\bm{v}=\alpha(\beta\bm{v})$では，
左辺の$\alpha\beta$は$K$の乗法であり，
それ以外の並置はすべてスカラー倍である．

なお，加法とスカラー倍を「終域が$V$である写像」として定めたので，
$\bm{v}+\bm{w}$や$\alpha\bm{v}$がふたたび$V$の元であること，
すなわち$V$がこれらの演算について閉じていることは，定義に含まれている．
...
さらに，第1章で導入した線型写像の像と核（\deref{像と核}）は，線型部分空間の重要な例を与える\footnote{第1章では$K$を$\mathbb{R}$または$\mathbb{C}$としていたが，線型写像とその像・核の定義（\deref{線型写像}，\deref{像と核}），および\prref{線型写像と零ベクトル}や\prref{線型写像と線型結合}などの基本性質は，一般の体$K$に対してもそのまま通用する．}．
...
一方，第1章では，ベクトルの組に「無駄がない」ことを表す概念として線型独立を導入した（\deref{線型独立}）．
...
第1章では，数ベクトル空間$K^m$に限って「線型独立な組の拡大」と「取替補題」を示した（\lemref{線型独立な組の拡大}，\lemref{取替補題}）．
本項では，同じ議論が一般の線型空間においてもそのまま通用することを確認する．
これは単なる繰り返しではなく，第1章の具体的な議論を抽象的な枠組みへ再構成する作業である．
...
\begin{mycolumn}
  ...
  定義の順序としては「基底$\to$（個数の一意性）$\to$次元」となっており，次元を先に定義することはできない点に注意してほしい．
\end{mycolumn}
...
\begin{proof}
  ...
  (i)$\Rightarrow$(iii)：\thref{基底の延長}より，この組はベクトルを付け加えて基底にできる．
  しかし\thref{基底の個数の一意性}より基底の個数は$n$でなければならないから，付け加えるベクトルは$0$個である．

  (ii)$\Rightarrow$(iii)：\thref{基底の存在}より，この組から適当な部分を取り出して基底にできる．
...
\begin{mycolumn}
  ...
  \]
  と同じ形をしている．
  ただし，線型空間では合併$W_1 \cup W_2$ではなく和$W_1 + W_2$を考える点が異なる（\prref{和と線型包}のあとの注意を参照）．
...
第1章で，線型写像$f \colon K^n \to K^m$に対して像$\Im f$と核$\Ker f$を定義した（\deref{像と核}）．
...
第1章で「線型独立な列の最大個数」として導入した階数（\deref{階数}）は，この公式を通じて「像の次元」として捉え直される．
...
一方，その座標ベクトルは，基底の選び方に応じて変化する「表示」である（\prref{座標ベクトルの性質}のあとのコラムを参照）．
...
\begin{mycolumn}
  ...
  この場合，$\bm{v} = P[\bm{v}]_{\mathcal{B}'}$という関係は，第1章のコラムで見た「$A\bm{x}$は$A$の列ベクトルの線型結合であり，その係数は$\bm{x}$の成分である」という事実にほかならない．

  また，$K = \mathbb{R}$または$\mathbb{C}$とする．第2章では行列式に関する諸性質を複素数行列について証明したが，実数行列についても同じ定義式と証明がそのまま通用する（第2章冒頭の注意を参照）．よって，\coref{次元による基底の判定}と\prref{行列式と階数}を合わせると，
