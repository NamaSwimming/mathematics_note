...
\begin{abstract}
  ...
  線型部分空間の重要な例としては，第1章で定義した線型写像の像と核を取り上げる．
  そのうえで基底と次元を定義し，部分空間の次元に関する次元公式，および像と核の次元を結びつける線型写像の次元公式を証明する．
\end{abstract}
...
一方，体$K$の乗法は，スカラー倍$\scalmul$と区別して，記号を省略し$\alpha\beta$のようにかく．
\begin{enumerate}[label=(\roman*),leftmargin=3.2em]
  ...
  \]
  \[
  (\alpha+\beta) \scalmul \bm{v}=\alpha \scalmul \bm{v}+\beta \scalmul \bm{v},
  \]
  \[
  \alpha \scalmul (\bm{v}+\bm{w})=\alpha \scalmul \bm{v}+\alpha \scalmul \bm{w},
  \]
  \[
  (\alpha\beta) \scalmul \bm{v}=\alpha \scalmul (\beta \scalmul \bm{v})
  \]
  が成り立つ．
  \end{enumerate}
...
以下では，スカラー倍の演算記号$\scalmul$を省略し，
\[
  \alpha\scalmul\bm{v}
  \quad\text{を単に}\quad
  \alpha\bm{v}
\]
とかく\footnote{記号$\scalmul$をいったん導入したのは，スカラー倍が$K\times V\to V$という，$K$の乗法$K\times K\to K$とは異なる演算であることを明示するためである．両者を混同するおそれのない場面では，体の乗法$\alpha\beta$と同様に記号を省略するのが慣習である．}．
...
なお，加法とスカラー倍を「終域が$V$である写像」として定めたので，$\bm{v}+\bm{w}$や$\alpha\bm{v}$がふたたび$V$の元であること，すなわち$V$がこれらの演算について閉じていることは，定義に含まれている．
