...
本章では議論を一般化し，$K$は任意の\textbf{体}\index{たい@体}（field）であるとする\footnote{体とは，大まかにいえば，加減乗除の四則演算が自由に行える数の体系のことである．$\mathbb{R}$や$\mathbb{C}$はその代表例であり，本書では，体の典型例として$\mathbb{R}$と$\mathbb{C}$を思い浮かべておけば十分である．}．
...
\begin{example}{基底の例}{基底の例}
  \begin{enumerate}[label=(\arabic*),leftmargin=3.2em]
    ...
    \item $m \times n$行列全体のなす線型空間において，$(i,j)$成分のみが$1$で他が$0$である行列$E_{ij}$の組は基底である．
...
\begin{mycolumn}
  ...
  実際，有限個の多項式$f_1, \dots, f_n$をとると，それらの線型結合の次数は$\max_i \deg f_i$を超えないので，より高次の多項式は生成できない．
...
\begin{proof}
  ...
  $\bm{v}$と$\bm{w}$の役割を入れ替えて同じ議論を行えば$n \le m$を得る．
  よって$m = n$である．
  これが証明すべきことであった．
\end{proof}
\end{leftbar}
...
\begin{mycolumn}
  ...
  \end{itemize}
  の二つがそろって初めて意味をもつ．
  \thref{基底の個数の一意性}は後者を保証するものであり，前者は次の\thref{基底の存在}が保証する．
...
\begin{proof}
  $\dim V = n$とする．
  \lemref{一般の取替補題}より，$W$の線型独立なベクトルの個数は高々$n$である．
  そこで，$W$の線型独立な組のうち，含まれるベクトルの個数が最大であるものを$(\bm{w}_1, \dots, \bm{w}_r)$とする（$r \le n$）．
...
\begin{proof}
  ...
  生成性について，\prref{和と線型包}より$W_1 + W_2 = \Span(W_1 \cup W_2)$であり，$W_1$も$W_2$も上の組の線型包に含まれるから，\prref{線型包の最小性}より従う．
...
\begin{mycolumn}
  ...
  この性質を，座標は基底に対して\textbf{反変的}\index{はんぺんてき@反変的}であるという．
\end{mycolumn}
