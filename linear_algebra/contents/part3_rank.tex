...
\begin{abstract}
  ...
  次に，自明な線型関係しかもたない組として\textbf{線型独立}を，そうでない組として\textbf{線型従属}を定義し，
  それぞれについて扱いやすい言い換えを与える．
  続いて，以後くり返し用いる「線型独立な組の拡大」と「取替補題」を証明する．
  最後に，行列の\textbf{階数}を線型独立な列の最大個数として定義し，
  行階数と列階数が一致すること，および行基本変形によって階数が計算できることを示す．
\end{abstract}
