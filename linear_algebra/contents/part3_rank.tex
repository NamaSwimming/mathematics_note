...
ここでは，\partref{行列式}までの議論で必要となる数ベクトル空間$K^m$の場合を先に示す．
...
\begin{proof}
  ...
  \]
  となり，$\bm{w}$が$\bm{v}_1, \bm{v}_2, \dots, \bm{v}_k$の線型結合として表せることになって仮定に反する．
...
\begin{proof}
  ...
  \]
  となり，この式より$\bm{v}_{k+1}$は
  $\bm{w}_1, \dots, \bm{w}_{k+1}, \bm{v}_{k+2}, \dots, \bm{v}_n$
  の線型結合として表すことができる．
...
\partref{線型空間}で次元を定義したあとでは，階数は列ベクトルの線型結合全体，すなわち像$\Im T_A$の次元として
\[
\rank A = \dim \Im T_A
\]
と表せる（\coref{階数と像の次元}）．
本章では次元がまだ定義されていないため，そのことばを使わずに同じ数を取り出す形で階数を定義している．
\end{remark}
...
\begin{proof}
  ...
  \[
    (\text{$A$の第$k$行}) = \sum_{i=1}^{r} (\text{$C$の$(k,i)$成分}) \cdot (\text{$R$の第$i$行})
  \]
...
行基本変形で得られる各式は$f_1, \dots, f_m$の線型結合であるから，
$A\bm{x} = \bm{0}$を満たす$\bm{x}$は$A’\bm{x} = \bm{0}$も満たす．

また，\deref{行基本変形}の$3$種類の操作はいずれも逆操作をもつ
（$c$倍の逆は$1/c$倍，行の入れ替えの逆は同じ入れ替え，第$j$行の$c$倍を加える操作の逆は$(-c)$倍を加える操作である）．
同じ議論を逆操作に適用すれば逆向きの包含も従う．
これが証明すべきことであった．
\end{proof}
\end{tleftbar}
...
$j_1 < \cdots < j_k$を任意にとり，$A$の第$j_1, \dots, j_k$列からなる行列を$B$，
$A’$の同じ列からなる行列を$B’$とする．
$A$から$A’$への行基本変形は$B$から$B’$への行基本変形を引き起こすから，
\prref{行基本変形と解集合}より$B\bm{y} = \bm{0}$と$B’\bm{y} = \bm{0}$の解集合は一致する．

\prref{列の線型関係と斉次連立方程式}より，第$j_1, \dots, j_k$列が線型独立であることは，
$B\bm{y} = \bm{0}$（$A’$については$B’\bm{y} = \bm{0}$）が自明な解しかもたないことと同値である．
よって，どの列の組についても$A$と$A’$で線型独立であるかどうかは一致する．
したがって線型独立な列の最大個数も一致し，$\rank A = \rank A’$である．
これが証明すべきことであった．
\end{proof}
\end{tleftbar}
...
\begin{remark}[行基本変形による階数の計算]
  ...
  最終的に，$0$でない各行の先頭の非零成分が右下に向かって階段
  \newpage 
  \noindent 
  状に並び，
